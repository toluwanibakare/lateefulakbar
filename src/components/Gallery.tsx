          ...
          <div className="mt-8 flex flex-wrap gap-2" role="tablist" aria-label="Filter gallery">
            {FILTERS.map((f) => (
              <button
                ...
                onClick={() => setFilter(f)}
                className={`px-4 py-2 text-[12px] font-semibold tracking-wide transition-colors ${
                  filter === f
                    ? "bg-vivid text-white"
              ...
              >
                <button onClick={() => setActive(g)} className="block w-full" aria-label={`View ${g.label}`}>
                  <Image
                    ...
                    className="img-true h-auto w-full object-cover transition-transform duration-700 group-hover:scale-[1.03]"
                    loading="lazy"
                  />
                </button>
        ...
        {active && (
          ...
          >
            <div className="flex items-center justify-between text-white">
              <p className="max-w-xl text-sm">{active.label}</p>
              <button aria-label="Close viewer" className="p-2 hover:text-sage">
                <X className="h-6 w-6" />
              </button>
            </div>
            <div className="relative mx-auto mt-4 w-full max-w-5xl flex-1" onClick={(e) => e.stopPropagation()}>
