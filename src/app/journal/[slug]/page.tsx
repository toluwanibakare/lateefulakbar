...
const BODIES: Record<string, string[]> = {
  "sea-of-white": [
    "Tafawa Balewa Square holds noise well — it was built for parades and crowds. On the day of the sitting, it held silence better. Forty thousand people in white, breathing the same dhikr, and the loudest thing for long stretches was water being passed hand to hand.",
  ...
  "ya-lateef": [
    ...
    "Come with ablution, come in white, come having forgiven one person. That is the whole preparation the founder asks of first-time guests.",
  ],
  "tbs-logistics": [
        ...
        <div className="mx-auto max-w-3xl px-5 py-14 sm:px-6 md:py-20">
          ...
          <Reveal delay={0.1}>
            <div className="mt-12 flex flex-wrap gap-3 border-t border-ink/10 pt-8">
              ...
              <Link href="/journal" className="border border-ink/20 px-6 py-3 text-sm font-semibold text-ink hover:border-pine hover:text-pine">
                ← All journal entries
              </Link>
            </div>
          </Reveal>
      ...
      <section className="border-t border-ink/10 bg-cream">
        <div className="mx-auto max-w-7xl px-5 py-14 sm:px-6">
          ...
          <div className="mt-6 grid gap-8 md:grid-cols-2">
            {others.map((p) => (
              <Link key={p.slug} href={`/journal/${p.slug}`} className="group grid gap-4 sm:grid-cols-[200px_1fr] sm:items-center">
